...
export async function POST(
  ...
  try {
    ...
    if (type === 'text') {
      ...
      if (!title) {
        return NextResponse.json(
          { error: 'Title is required for text content' },
          { status: 400 }
        )
      }
    } else if (type === 'url') {
      ...
      if (!isValidWebUrl(sourceUrl)) {
        return NextResponse.json(
          {
            error:
              'Invalid or unsafe URL. Please provide a valid HTTP/HTTPS URL.',
          },
          { status: 400 }
        )
      }
      ...
      if (!title || title.trim() === '') {
        title =
          scrapedResult.title || `Content from ${extractDomain(sourceUrl)}`
      }

      console.log(
        `✅ Successfully scraped ${scrapedResult.wordCount} words from ${extractDomain(sourceUrl)}`
      )
    } else if (type === 'file') {
      ...
      try {
        ...
        console.log(
          `✅ Successfully extracted ${content.length} characters from ${file.name}`
        )
      } catch (extractError) {
    ...
    if (sourceType === 'URL' && scrapedResult) {
      ...
      console.log(
        `🖼️ Scraped result contains ${scrapedResult.images?.length || 0} images`
      )
      console.log(
    ...
    if (
      (sourceType === 'URL' || content.length > 100) &&
      process.env.OPENAI_API_KEY
    ) {
      try {
    ...
    if (existingDoc) {
      ...
      knowledgeDoc = await db.knowledgeDoc.update({
        ...
        data: {
          ...
          sourceType,
          metadata:
            Object.keys(enhancedKnowledge.metadata || {}).length > 0
              ? enhancedKnowledge.metadata
              : null,
          updatedAt: new Date(),
        },
      })
    ...
    try {
      ...
      for (const chunk of chunks) {
        try {
          ...
          await storeEmbedding(knowledgeDoc.id, chunk.content, embedding, {
      ...
      console.log(
        `✅ Successfully created ${embeddingsCreated} embeddings for knowledge document`
      )
    } catch (error) {
      console.error('Error generating embeddings:', error)
      console.log(
        '📄 Knowledge document saved but embeddings failed - helpers will use text search fallback'
      )
    }
    ...
    if (
      ...
      metadata.processedImages.length > 0
    ) {
      console.log(
        `🖼️ Creating individual knowledge documents for ${metadata.processedImages.length} images...`
      )
        ...
        try {
          ...
          if (existingImageDoc) {
            ...
            imageDoc = await db.knowledgeDoc.update({
              ...
              data: {
                content:
                  image.aiDescription || image.alt || 'Image from website',
                metadata: {
                  imageUrl: image.src,
                  altText: image.alt,
                  width: image.width,
          ...
          console.error(
            `Error creating knowledge document for image ${i + 1}:`,
            error
          )
      ...
      console.log(
        `✅ Successfully created/updated ${imageKnowledgeDocs.length} image knowledge documents`
      )
    }
